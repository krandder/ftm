...
Conversion between the Python model variables and the JavaScript model variables
...

{
  let python_to_js_keys = {
    ...
    'initial_cognitive_share_hardware_rnd':                'initial.hardware_rnd.share.cognitive',
    'initial_compute_share_goods':                         'initial.goods.share.compute',
    'initial_compute_share_rnd':                           'initial.rnd.share.compute',
    'initial_experiment_share_software_rnd':               'initial.software_rnd.share.experiments',
    ...
    'use_lognormal_task_distribution':                     'use_lognormal_task_distribution',
  ...
  let internal_variables = {
    ...
    'money_spent_training':               'money_spent_training',
    'compute_software_rnd_experiments':   'software_rnd.experiments_compute',
  };
  ...
  function get_external_variables(sim, step) {
    ...
    let getters = {
      "rampup":                       () => state.rampup,
      "hardware performance":         () => nj.log10(state.hardware_performance.v),
      "frac_gwp_compute":             () => state.frac_gwp.compute.v,
      "hardware":                     () => nj.log10(state.hardware),
      "software":                     () => state.software.v,
      "compute":                      () => nj.log10(state.compute),
      "labour":                       () => state.labour / state0.labour,
      "capital":                      () => state.capital / state0.capital * 0.025,
      "automatable tasks goods":      () => ftm.Model.get_automatable_task_count(state, consts, 'goods') + 1,
      "frac automatable tasks goods": () => ftm.Model.get_automatable_task_count(state, consts, 'goods')/consts.goods.n_labour_tasks,
      "automatable tasks rnd":        () => ftm.Model.get_automatable_task_count(state, consts, 'rnd') + 1,
      "frac automatable tasks rnd":   () => ftm.Model.get_automatable_task_count(state, consts, 'rnd')/consts.rnd.n_labour_tasks,
      "gwp":                          () => state.gwp,
      ...
      "ratio rnd input hardware":     () => state0.hardware_performance.rnd_input**consts.rnd.parallelization_penalty / state0.hardware_performance.cumulative_rnd_input,
      "biggest_training_run":         () => nj.log10(state.biggest_training_run),
  ...
  // Exports
  // ---------------------------------------------------------------------------
  bridge.transform_python_to_js_params = transform_python_to_js_params;
  bridge.get_internal_variables = get_internal_variables;
  bridge.get_external_variables = get_external_variables;
  bridge.get_takeoff_metrics    = get_takeoff_metrics;
}
