...
metrics_to_show.addEventListener('change',  () => {
  ...
  if (ui_state.metrics_to_show == 'important-metrics') {
    let important_metrics = ['automation_gns_99%', 'automation_gns_99.9%', 'automation_gns_100%', 'full_automation_gns', 'full_automation_rnd'];
...
function import_scenario(params) {
  for (let param in params) {
    ...
    if (param == 'runtime_training_tradeoff_enabled') {
      ...
      input.checked = Boolean(params[param]);
    } else {
...
metric_names['automation_gns_99%'] = '99% automation year';
metric_names['automation_gns_99.9%'] = '99.9% automation year';
metric_names['automation_rnd_99%'] = '99% R&D automation year';
metric_names['automation_rnd_99.9%'] = '99.9% R&D automation year';
metric_meanings['automation_gns_99%'] = 'Year when AI has automated 99% of goods and services tasks.';
metric_meanings['automation_gns_99.9%'] = 'Year when AI has automated 99.9% of goods and services tasks.';
metric_meanings['automation_rnd_99%'] = 'Year when AI has automated 99% of R&D tasks (in software and hardware).';
metric_meanings['automation_rnd_99.9%'] = 'Year when AI has automated 99.9% of R&D tasks (in software and hardware).';
...
function draw_requirements(runtime_or_training, graph_container, steepness_node) {
  ...
  let automation_buckets = ftm.Model.process_automation_buckets(
    full_requirements,
    gap,
    n_labour_tasks,
    steepness,
    params['use_lognormal_task_distribution'],
  );
  let automation_costs = automation_buckets.requirements;
  let task_masses = automation_buckets.masses;
  ...
  for (let i = 1; i < n_labour_tasks + 1; i++) {
    cumulative_mass += task_masses[i - 1];
    percentage_automated[i] = 100 * cumulative_mass;
  }
